...
static esp_err_t data_handler(httpd_req_t *req) {
  ...
  String json_response = String("{\"temperature\":\"") + String(temperature) + String("\",");
  json_response += String("\"humidity\":\"") + String(humidity) + String("\",");
  json_response += String("\"water\":\"") + String(water) + String("\",");
  json_response += String("\"light\":\"") + String(light) + String("\"}");
  httpd_resp_set_type(req, "application/json");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, json_response.c_str(), json_response.length());
}

static esp_err_t temperature_handler(httpd_req_t *req) {
  // POST request handler for temperature setpoint
  char content[100];
  ...
  uint8_t value = convertASCIItoInteger((uint8_t *)content, req->content_len);
...
static esp_err_t humidity_handler(httpd_req_t *req) {
  // POST request handler for humidity setpoint
  char content[100];
  size_t recv_size = min(req->content_len, sizeof(content));
  int ret = httpd_req_recv(req, content, recv_size);
  if (ret <= 0) {
...
static esp_err_t light_handler(httpd_req_t *req) {
  // POST request handler for lighting setpoint
  char content[100];
  size_t recv_size = min(req->content_len, sizeof(content));
  int ret = httpd_req_recv(req, content, recv_size);
  if (ret <= 0) {
...
static esp_err_t water_handler(httpd_req_t *req) {
  Serial.println("Inside water handler");
  // POST request handler for watering setpoint
  char content[100];
  size_t recv_size = min(req->content_len, sizeof(content));
  int ret = httpd_req_recv(req, content, recv_size);
  if (ret <= 0) {
  ...
  uint8_t value = convertASCIItoInteger((uint8_t *)content, req->content_len);
  Serial.println("Water setpoint time = " + String(value));
  uint8_t arr[7];
...
void startCameraServer(){
   ...
   httpd_uri_t stream_uri = {
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &water_uri);
